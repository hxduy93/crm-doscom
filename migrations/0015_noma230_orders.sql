-- Đơn đăng ký từ landing NOMA 230 (xịt dưỡng & đánh bóng nhựa nhám).
-- Repo landing: hxduy93/noma230-landing → Pages Function /api/order → endpoint này.
--
-- Vì sao KHÔNG dùng lại bảng có sẵn (cùng lý do đã ghi ở 0013 cho 680, 0014 cho 350):
--   * noma911_orders / noma680_orders gắn cứng combo của dòng sản phẩm đó. Trộn 230 vào
--     sẽ làm lẫn by_combo của thống kê đang chạy.
--   * landing_leads CỐ Ý không có combo/gift/amount/POS (xem comment 0011), dành cho
--     landing chỉ thu lead. 230 bán theo combo có giá nên nhét vào là mất doanh thu.
--
-- Khuôn giữ ĐÚNG như noma680_orders để /api/noma230/stats trả cùng hình dạng response,
-- dashboard tái dùng được component sẵn có.

CREATE TABLE IF NOT EXISTS noma230_orders (
  id            INTEGER PRIMARY KEY AUTOINCREMENT,
  staff         TEXT    NOT NULL,          -- 'duy' | 'pn' (khớp noma911_orders, noma680_orders)
  combo         TEXT    NOT NULL,          -- 'le-230' | 'combo-2x230' | 'combo-230-911' | 'combo-230-680'
  combo_label   TEXT,                      -- nhãn hiển thị, server tự chốt
  gift          TEXT,                      -- 'khan-microfiber' | 'noma250' | '' (đơn lẻ không quà)
  name          TEXT,                      -- form 230 có thu tên
...
CREATE INDEX IF NOT EXISTS idx_noma230_date  ON noma230_orders (created_date);
CREATE INDEX IF NOT EXISTS idx_noma230_phone ON noma230_orders (phone);
CREATE INDEX IF NOT EXISTS idx_noma230_staff ON noma230_orders (staff);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_noma230_dedup
  ON noma230_orders (created_date, phone, combo);
