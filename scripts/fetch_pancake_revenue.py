...
CHUNK_DAYS = 7
FETCH_WORKERS = 4        # số lát quét song song (xem fetch_orders_by_group)
...
def _fetch_one_chunk(args):
    """Quét 1 lát thời gian → (ci, danh sách đơn thô của lát).

    Chạy trong thread (xem FETCH_WORKERS). Không đụng state chung — gộp/dedup
    để phần gọi làm, cho dễ suy luận.
    """
...
def fetch_orders_by_group(start_dt, end_dt):
    ...
    FETCH_WORKERS lát chạy song song — runner GitHub gọi Pancake chậm hơn máy
    local ~4 lần, chạy tuần tự thì quá 25 phút và bị hủy giữa chừng (đo
    2026-08-10: run #31353785425 chết ở lát 12/13).
    Đơn ngoài 6 nhóm bị bỏ ngay khi gộp — window 90 ngày có ~46k đơn nhưng chỉ
    ~10k thuộc nhóm nào đó. Dedup theo order.id để phòng trùng ở ranh giới lát.
    ...
    jobs = [(ci, len(chunks), c_start, c_end)
...
def order_revenue(order):
    """
    Doanh thu per order = SỐ TIỀN COD của đơn (đúng cột "doanh số" trên Pancake POS).
    (QUYẾT 2026-07-15, đã kiểm chứng trên data thật cả 3 nguồn Duy/Phương Nam/Website:
     sum(cod) khớp Pancake TUYỆT ĐỐI.)
    Dùng THẲNG trường cod — kể cả cod=0 (đơn trả trước / giá trị 0) thì tính 0,
    KHÔNG fallback sang total_price/after_sub (fallback sẽ thổi các đơn cod=0 lên,
    làm Website lệch +497k so Pancake). Trả về float (giữ số lẻ).
    """
