...
export async function onRequestPost(context) {
  ...
  // Endpoint MỞ (theo yêu cầu): CRM chưa có đăng nhập, không kiểm tra session.
  // Lưu ý: ai biết URL đều gọi được → tốn token Claude. Cân nhắc thêm token sau.
  ...
  // Call AI: CHỈ dùng Claude Haiku (đã bỏ fallback Llama). Claude lỗi → trả lỗi.
  let aiResult;
  ...
  // CHỈ chạy Claude Haiku (đã BỎ fallback Llama theo yêu cầu).
  // Claude không chạy được → trả lỗi rõ ràng, KHÔNG âm thầm dùng Llama.
  if (!claudeUsed) {
    const isCredit = claudeError && /credit balance is too low|Plans & Billing|insufficient|billing/i.test(claudeError);
    const noKey = !env.ANTHROPIC_API_KEY || !env.CF_ACCOUNT_ID;
    return jsonResponse({
      error: isCredit
        ? "Tài khoản Anthropic chưa đủ credit để dùng Claude Haiku. Vào console.anthropic.com → Plans & Billing để nạp credit (tối thiểu ~$5)."
        : noKey
          ? "Chưa cấu hình Claude: thiếu ANTHROPIC_API_KEY hoặc CF_ACCOUNT_ID trên Cloudflare Pages."
          : claudeError
            ? `Claude Haiku lỗi: ${claudeError}`
            : "Claude Haiku không khả dụng cho mode này.",
      claude_error: claudeError,
      claude_debug: claudeDebug,
      need_credit: !!isCredit,
    }, 502);
  }
