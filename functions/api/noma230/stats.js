// GET /api/noma230/stats?days=90  (hoặc &from=YYYY-MM-DD&to=YYYY-MM-DD)
// Thống kê đơn landing NOMA 230 từ D1 riêng của CRM (binding DB = crm-doscom-db).
//
// Hình dạng response giữ ĐÚNG như /api/noma911/stats và /api/noma680/stats để dashboard
...
export async function onRequestGet(context) {
  ...
  try {
    const [summary, byCombo, byStaff, byGift, bySource, byDate, actual] = await Promise.all([
      env.DB.prepare(`SELECT COUNT(*) AS orders, COUNT(DISTINCT NULLIF(phone,'')) AS unique_customers, COALESCE(SUM(amount),0) AS revenue FROM noma230_orders WHERE ${where}`).bind(...args).first(),
      env.DB.prepare(`SELECT combo, combo_label, COUNT(*) AS orders, COALESCE(SUM(amount),0) AS revenue FROM noma230_orders WHERE ${where} GROUP BY combo, combo_label ORDER BY orders DESC`).bind(...args).all(),
      env.DB.prepare(`SELECT staff, COUNT(*) AS orders, COALESCE(SUM(amount),0) AS revenue FROM noma230_orders WHERE ${where} GROUP BY staff ORDER BY orders DESC`).bind(...args).all(),
      env.DB.prepare(`SELECT CASE WHEN gift IS NULL OR gift='' THEN '(không quà)' ELSE gift END AS gift_key, COUNT(*) AS orders FROM noma230_orders WHERE ${where} GROUP BY gift_key ORDER BY orders DESC`).bind(...args).all(),
      env.DB.prepare(`SELECT source, COUNT(*) AS orders FROM noma230_orders WHERE ${where} GROUP BY source ORDER BY orders DESC`).bind(...args).all(),
      env.DB.prepare(`SELECT created_date, COUNT(*) AS orders, COALESCE(SUM(amount),0) AS revenue FROM noma230_orders WHERE ${where} GROUP BY created_date ORDER BY created_date ASC`).bind(...args).all(),
      env.DB.prepare(`SELECT COALESCE(SUM(CASE WHEN pos_status=3 THEN pos_cod ELSE 0 END),0) AS rev_delivered, COALESCE(SUM(CASE WHEN pos_status NOT IN (4,5,6) THEN pos_cod ELSE 0 END),0) AS rev_booked, SUM(CASE WHEN pos_status=3 THEN 1 ELSE 0 END) AS cnt_delivered, SUM(CASE WHEN pos_status NOT IN (4,5,6) THEN 1 ELSE 0 END) AS cnt_booked, COUNT(*) AS cnt_matched FROM (SELECT pos_order_id, pos_status, pos_cod FROM noma230_orders WHERE ${where} AND pos_matched=1 AND pos_order_id IS NOT NULL GROUP BY pos_order_id)`).bind(...args).first(),
    ]);
