// POST /api/noma230/order
// Nhận 1 đơn từ landing NOMA 230 (repo noma230-landing → Pages Function /api/order).
//
...
export async function onRequestPost({ request, env }) {
  ...
  // Token RIÊNG của 230 — không dùng chung NOMA911_INGEST_TOKEN hay NOMA680_INGEST_TOKEN.
  // Lộ token landing này không được phép kéo theo landing kia.
  if (request.headers.get("X-Noma-Token") !== env.NOMA230_INGEST_TOKEN) {
  ...
  try {
    ...
    const res = await env.DB.prepare(
      `INSERT OR IGNORE INTO noma230_orders
