// GET /api/health/token-alert
// Cảnh báo hạn token / key chết cho banner trên dashboard. 2 nguồn:
//   1. Token FB sắp hết hạn (≤2 ngày) hoặc đã chết — kiểm LIVE bằng FB debug_token.
...
//
// Trước đây nằm ở /api/hermes/alerts và chỉ hiện trong widget chat Hermes.
// Hermes đã gỡ khỏi CRM → tách ra endpoint riêng để cảnh báo token KHÔNG mất
// (token FB chết = dashboard âm thầm ngừng cập nhật số liệu).
//
...
// Trả về mảng câu ngắn để chèn thêm vào banner. Bỏ FB (đã kiểm riêng bên dưới).
...
export async function onRequestGet(context) {
  ...
  // Cache FB (tránh gọi FB mỗi lần trang load). Vẫn phải gộp health bên dưới,
      ...
      if (invalid || (days !== null && days <= WARN_DAYS)) {
        ...
        payload.alert = {
          ...
          msg: invalid
            ...
            : `⚠️ Token Facebook còn ${dRound} ngày (hết hạn ${expStr}). Làm mới sớm để dashboard không gián đoạn.`,
        };
      }
