// Test tự động cho agent noma350 — cùng bốn luật với tests/noma680.test.mjs.
// Chạy bằng Node có sẵn (KHÔNG cần cài thêm gì):  node --test tests/
// Chỉ ĐỌC endpoint công khai /api/noma350/stats — không sửa, không ghi gì.
//
...
// Giá cố định từng combo (lấy đúng từ COMBO_META trong functions/api/noma350/order.js)
const GIA_COMBO = {
  "le-350": 159000,
  "combo-2x350": 318000,
  "combo-350-911": 378000,
  "combo-350-922": 378000,
};

// Gọi API 1 lần. Trả null nếu endpoint chưa deploy để test tự bỏ qua.
async function layThongKe() {
  const res = await fetch(`${BASE_URL}/api/noma350/stats?days=365`);
...
const CHUA_DEPLOY = "endpoint /api/noma350/stats chưa deploy — bỏ qua (bình thường ở lần deploy đầu)";

// ── TEST 1: CONTRACT — kết quả có đúng khuôn không? ──────────────────────
test("noma350/stats trả về đúng cấu trúc (contract)", async (t) => {
...
test("by_combo chỉ chứa combo của NOMA 350", async (t) => {
  const kq = await layThongKe();
  if (!kq) return t.skip(CHUA_DEPLOY);
