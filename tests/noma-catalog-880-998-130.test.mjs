...
test("giá khớp PRICING của landing đang chạy", () => {
  ...
  // Luật quà mới 24/09/2026: gói 2 chai 358.000đ (dưới 400k) tặng NOMA 110, bỏ "mua 2 tặng 1".
  assert.match(PRODUCTS["Noma 130"].priceRange, /2 chai 358\.000đ/);
  assert.match(PRODUCTS["Noma 130"].priceRange, /NOMA 110/);
  assert.doesNotMatch(PRODUCTS["Noma 130"].priceRange, /mua 2 tặng 1/i);
  assert.match(PRODUCTS["Noma 880"].priceRange, /NOMA 955/);
  assert.match(PRODUCTS["Noma 998"].priceRange, /gói 4 chai tặng 1 chai NOMA 955/);
});
